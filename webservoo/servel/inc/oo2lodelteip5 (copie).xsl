...
<xsl:stylesheet
  ...
  xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:param name="verbose">false</xsl:param><!-- TODO false -->
    ...
    <xsl:template match="text:h[@text:outline-level]">
        ...
        <xsl:variable name="precedingLevel">
            <xsl:choose>
                <xsl:when test="preceding::text:h[@text:outline-level]">
                    <xsl:value-of select="preceding::text:h[@text:outline-level][1]/@text:outline-level"/>
                </xsl:when>
                <xsl:otherwise>
                <xsl:text>0</xsl:text>
            </xsl:otherwise>
        ...
        <xsl:if test="$verbose='true'">
            ...
            <xsl:message>|level <xsl:value-of select="$level"/>, following <xsl:value-of select="$precedingLevel"/>, <xsl:value-of select="."/></xsl:message>
        </xsl:if>
        ...
        <xsl:if test="$verbose='true'">
            <xsl:message>|found a gap in hierarchy: <xsl:value-of select="$level"/> follows <xsl:value-of select="$precedingLevel"/>: <xsl:value-of select="($level - $precedingLevel)-1"/></xsl:message>
        </xsl:if>
