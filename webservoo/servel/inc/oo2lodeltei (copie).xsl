...
<!-- office:text -->
    ...
    <xsl:template match="text:p[@text:style-name]">
        <xsl:choose>
            ...
            <xsl:when test="@text:style-name='documenttitle'">
                <title>
                <xsl:apply-templates/>
                </title>
            </xsl:when>
            ...
            <xsl:when test="@text:style-name='sectiontitle'">
                <head>
                <xsl:apply-templates/>
                </head>
            </xsl:when>
            <xsl:when test="@text:style-name='appendixtitle'">
                <head>
                <xsl:apply-templates/>
                </head>
            </xsl:when>
    ...
    <xsl:template match="text:span">
        ...
        <xsl:choose>
            ...
            <xsl:when test="$Style='filespec'">
                <Filespec><xsl:apply-templates/></Filespec>
            </xsl:when>
            <xsl:when test="$Style='gi'">
                <gi><xsl:apply-templates/></gi>
            </xsl:when>
                ...
                <xsl:for-each select="key('STYLES',$name)">
                    ...
                    <xsl:choose>
                        <xsl:when test="style:text-properties[starts-with(@style:text-position,'super')]">
                            <hi rend="sup"><xsl:copy-of select="$contents"/></hi>
                        </xsl:when>
                        <xsl:when test="style:text-properties[starts-with(@style:text-position,'sub')]">
                            <hi rend="sub"><xsl:copy-of select="$contents"/></hi>
                        </xsl:when>
                        <xsl:when test="style:text-properties[@fo:font-style='italic']">
                            <hi rend="italic"><xsl:copy-of select="$contents"/></hi>
                        </xsl:when>
                        <xsl:when test="style:text-properties[@fo:font-weight='bold']">
                            <hi rend="bold"><xsl:copy-of select="$contents"/></hi>
                        </xsl:when>
                        <xsl:when test="style:text-properties[@style:text-underline-style='solid']">
                            <hi rend="underline"><xsl:copy-of select="$contents"/></hi>
                        </xsl:when>
                        <xsl:when test="style:text-properties[@fo:font-variant='small-caps']">
                            <hi rend="small-caps"><xsl:copy-of select="$contents"/></hi>
                        </xsl:when>
                        <xsl:when test="style:text-properties[@fo:text-transform='uppercase']">
                            <hi rend="uppercase"><xsl:copy-of select="$contents"/></hi>
                        </xsl:when>
                        <xsl:when test="style:text-properties[@fo:text-transform='lowercase']">
                            <hi rend="lowercase"><xsl:copy-of select="$contents"/></hi>
                        </xsl:when>
                        <xsl:when test="style:text-properties[@fo:text-align='center']">
                            <hi rend="center"><xsl:copy-of select="$contents"/></hi>
                        </xsl:when>
                        <xsl:when test="style:text-properties[@fo:language]">
                            <xsl:variable name="lang">
                                <xsl:value-of select="style:text-properties/@fo:language"/>
                            </xsl:variable>
                            <hi xml:lang="{$lang}"><xsl:copy-of select="$contents"/></hi>
                        </xsl:when>
                        <xsl:otherwise>
